        ...
        }
        {
                ...
                .map_err(core_err)?
            {
                ...
                return Ok(Err(MutationError::RetirementBlocked {
                    reason: "template or auth dependency is retiring or unavailable".into(),
                }));
            }
            ...
            // verdict on the new spec, not a member-row count. Shared routing
            // owns its immutable members on the pool revision, not the Fleet.
            ...
            // Read only this field's stored contract; admission owns the full
            // spec schema, including provider-specific fields and legacy forms.
            #[derive(serde::Deserialize)]
            struct Inputs {
                #[serde(default)]
                template_inputs: serde_json::Map<String, serde_json::Value>,
            }
            let inputs_changed = previous
                .as_ref()
                .map(|p| {
                    let old: Inputs = serde_json::from_str(&p.spec_json)
                        .map_err(|e| core_err(crate::StoreError::Corrupt(e.to_string())))?;
                    let new: Inputs = serde_json::from_str(&facts.spec_json)
                        .map_err(|e| core_err(crate::StoreError::Corrupt(e.to_string())))?;
                    Ok::<_, shaula_core::error::CoreError>(
                        old.template_inputs != new.template_inputs,
                    )
                })
                .transpose()?
                .unwrap_or(false);
            if template_changed || auth_changed || inputs_changed {
                let occupancy = fleet_replacement_occupancy_in_tx(&tx, &facts.resource_key)
                    .await
                    .map_err(core_err)?;
                if occupancy > 0 {
