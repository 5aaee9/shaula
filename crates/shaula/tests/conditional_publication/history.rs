...
#[tokio::test]
async fn frozen_history_replays_before_mixed_conditions_and_current_head_checks() -> TestResult {
    ...
    for (kind, hash) in GOLDENS {
        ...
        let (etag, response) = accepted(
            fixture.app.clone().oneshot(request).await?,
            StatusCode::ACCEPTED,
        )
        .await?;
        assert_eq!(etag, format!("\"{}\"", original.etag), "{kind:?}");
        assert_eq!(response["changeId"], original.change.id, "{kind:?}");
        assert_eq!(response["revision"], 1);
        assert_eq!(kind.revision(&fixture).await?, 2);

        // A hash conflict (Fleet/Pool) or protected-material mismatch (profiles)
        // wins over the new mixed-condition rejection. No new write is allowed.
        let mut conflict = common::put_with_idempotency(
            &kind.uri(),
            "legacy-fixed",
            kind.body(digest, 2)?.to_string(),
        );
        conflict
            .headers_mut()
            .insert("if-match", "\"historic:7\"".parse()?);
        let response = fixture.app.clone().oneshot(conflict).await?;
        assert_eq!(response.status(), StatusCode::CONFLICT, "{kind:?}");
        ...
        for secret in ["rotated-kubeconfig", "rotated-private-key", "rotated-token"] {
