...
- GitHub Control-Plane Credentials 永不进入 Template/Terraform/Runner；sensitive Template bindings 只从 exact Revision 解析到获准 IaC child，以及 [ARD-0024](0024-bootstrap-official-runner-images-outside-containers.md) 明确规定的固定宿主 bootstrap child，并且永不进入 Runner/workflow。受支持的 v2 App 和 Template binding secrets 不会因 `write-only` 而丧失 restart/Destroy/recovery 可用性；旧认证执行必须在部署前按 spec 0018 用上一版本恢复，保留旧 bytes 不授权当前执行。
