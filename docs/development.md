...
`VITE_` variables. See [OIDC deployment](oidc-deployment.md).

## UI components

Run the `npx shadcn` commands in this section from `web/`, where
`components.json` lives. All other commands in this guide run from the repository
root.

`web/components.json` selects the official New York style. `web/src/components/ui/`
...
`/templates/new`, `/templates/{key}/revisions/new`, `/auth`, `/changes`,
`/jobs`, `/jobs/{id}`, `/jobs/runners`, and `/jobs/runners/{id}` support
...
Jobs and retained Apply/Destroy logs, and lookup/polling of accepted changes. The authentication page lists existing
