...
Full Template/Auth PUT follows the replay-before-mixed-condition rejection and
read-only replay recheck rules in [spec 0002 §5](0002-fleet-http-control-plane.md#5-http-interface).
Supported-format checks precede replay; replay must not restore a retired Auth
input format. Accepted historical Template results do not re-enter today's
new-publication Artifact admission. Secret comparison still uses the exact
recorded immutable Revision, not the current head.
...
[Spec 0022](0022-proxmox-runner-template.md) adds an explicit operator-managed Proxmox VM image
contract with an empty image set. Its artifact and runtime policy bind that trust declaration;
the unchanged attestation subject must match the empty set and cannot prove immutable VM contents.
All manifests without this opt-in retain the existing nonempty content-pin requirement.
...
A Fleet submits a bare Profile key（follow-only，[spec 0023](0023-fleet-template-follow-latest.md)；ARD-0029 之前落库的 `{key, revision}` 形式读取时归一化为 key）。A new or changed reference must resolve the current `active_revision`; an unchanged reference in a capacity/inputs update or no-op is not new-reference admission and keeps the retained pin. Spec 0002 §4.1 owns that distinction. An already-admitted Fleet keeps using its retained revision pin for normal reconciliation, future Create, Destroy and recovery until a cascade upgrade or reference change replaces it. Publishing or attesting a newer Revision upgrades follower Fleets through the spec 0023 cascade behind the zero-Resource-Occupancy rule; Runner Generations never change.
