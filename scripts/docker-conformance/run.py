...
from safety import (Rejected, admit_destroy_identity, admit_docker_engine, admit_plan, admit_removal, digest, inspect_container,
                    lock_providers, managed_resources, require, runner_identity)
from support import Commands, now, private_bytes, private_directory, private_json, save, save_json, source_digest
...
def prepare(args, commands, workspace, evidence, report):
    ...
    require(not list(workspace.glob("*.tfvars*")), "workspace_must_not_auto_load_inputs")
    frozen = private_bytes(Path(args.vars))
    inputs = json.loads(frozen)
    require(set(inputs) == {"shaula"}, "input_envelope")
    shaula = inputs["shaula"]
    require(isinstance(shaula.get("jit_config"), str) and shaula["jit_config"], "jit_missing")
    require(shaula.get("bindings", {}).get("docker_host") == args.socket, "docker_binding_mismatch")
    ...
    image = args.image
    require(image and re.fullmatch(r"[^\s@]+@sha256:[a-f0-9]{64}", image), "pinned_image_required")
    require(image in (workspace / "profile.yaml").read_text(), "image_not_in_manifest")
    journal = {"generation_id": generation["id"], "source_digest": source_digest(workspace),
               "input_digest": digest(frozen), "create_possible": False, "image": image,
    ...
    report.update({"kind": "shaula-docker-external-smoke/v1", "started_at": now(),
                   "full_conformance_passed": False,
                   "runtime": "external Terraform subprocess with protected local state",
                   "source_digest": journal["source_digest"], "runner_image": image,
                   "generation_id": generation["id"], "checks": {},
                   "not_proven": ["shaula exec Driver and independent job worker",
                   ...
                   "accepted_limitations": ["same-identity IaC children share Docker host authority",
    ...
    server = commands.docker_json("version", "--format", "{{json .Server}}", phase="docker_engine_version")
    admit_docker_engine(server)
    report["docker_engine"] = {"version": server.get("Version"), "api_version": server.get("ApiVersion"),
                               "minimum_api_version": server.get("MinAPIVersion"), "os": "linux"}
    daemon_id = commands.docker_json("info", "--format", "{{json .ID}}", phase="docker_daemon_identity")
    require(isinstance(daemon_id, str) and 0 < len(daemon_id) <= 128, "docker_daemon_identity_missing")
    journal["docker_daemon_id"] = daemon_id
    image_info = commands.docker_json("image", "inspect", image, phase="image_inspect")[0]
    ...
    report["engine"] = {"kind": "terraform", "version": version["terraform_version"],
    ...
    require(source_digest(workspace) == journal["source_digest"], "init_changed_artifact")
    commands.tf("plan", "-input=false", "-lock-timeout=30s", "-var-file=.docker-conformance/input.tfvars.json",
                "-out=.docker-conformance/create.tfplan", phase="create_plan")
    plan = commands.tf_json("show", "-json", ".docker-conformance/create.tfplan", phase="create_plan_show")
    admit_plan(plan, "create", [])
    resource = next(item for item in plan["resource_changes"] if item["mode"] == "managed")
    after = resource["change"]["after"]
    require(after.get("must_run") is False and after.get("rm") is False, "template_lifecycle_flags")
    ...
    commands.tf("apply", "-input=false", "-lock-timeout=30s", ".docker-conformance/create.tfplan", phase="create_apply")
    state = commands.tf_json("show", "-json", phase="create_state")
    resources = managed_resources(state)
    require(len(resources) == 1 and resources[0]["address"] == "docker_container.runner", "created_state_shape")
    journal["container_id"] = resources[0]["values"]["id"]
    require(re.fullmatch(r"[a-f0-9]{64}", journal["container_id"]), "container_id_shape")
...
def inspect(args, commands, journal, inputs, report):
    info = commands.docker_json("container", "inspect", journal["container_id"], phase="container_inspect")[0]
    inspect_container(info, journal["name"], journal["image_id"], inputs["shaula"]["jit_config"])
    require(info.get("State", {}).get("Running") is True, "container_not_running_for_handoff_check")
    commands.docker_run("exec", journal["container_id"], "/bin/sh", "-c", "test ! -e /shaula/jit_config",
                        phase="jit_file_consumed")
    argv = commands.docker_run("top", journal["container_id"], "-eo", "pid,args", phase="process_argv")
    ...
    report["checks"].update({"docker_security_shape": "passed", "jit_staged_file_consumed": "passed",
...
def destroy(args, commands, workspace, evidence, journal, report):
    require(not journal.get("destroy_possible"), "destroy_uncertain_manual_recovery_required")
    ...
    resources = managed_resources(state)
    require(len(resources) == 1 and resources[0]["values"]["id"] == journal["container_id"], "destroy_state_ownership")
    commands.tf("plan", "-destroy", "-input=false", "-lock-timeout=30s",
                "-var-file=.docker-conformance/input.tfvars.json", "-out=.docker-conformance/destroy.tfplan",
                phase="destroy_plan")
    plan = commands.tf_json("show", "-json", ".docker-conformance/destroy.tfplan", phase="destroy_plan_show")
    ...
    commands.tf("apply", "-input=false", "-lock-timeout=30s", ".docker-conformance/destroy.tfplan", phase="destroy_apply")
    ...
    require(not commands.tf("state", "list", phase="destroy_state_list").strip(), "destroy_state_not_empty")
    containers = commands.docker_run("container", "ls", "--all", "--no-trunc", "--quiet", "--filter",
                                    "id=" + journal["container_id"], phase="destroy_container_absence")
    require(not containers.strip(), "destroy_container_still_present")
    journal["destroy_completed"] = True
    save_json(evidence / "journal.json", journal)
    report["checks"].update({"safe_github_removal_receipt": "operator_observed",
...
def main():
    ...
    parser.add_argument("--image", help="Exact manifest image including digest; required for prepare")
    parser.add_argument("--removal-evidence", help="Owner-only sanitized GitHub absence receipt")
    ...
    require(not report_path.is_relative_to(workspace) or report_path.is_relative_to(evidence),
    ...
    with (evidence / "harness.lock").open("a") as mutex:
        ...
        try:
            if args.phase == "prepare":
                ...
                require(journal.get("create_completed") is True, "create_uncertain_manual_recovery_required")
                require(not journal.get("destroy_completed"), "already_destroyed")
                require(source_digest(workspace) == journal["source_digest"], "template_changed")
                require(digest(Path(commands.terraform).read_bytes()) == journal["engine_digest"], "engine_changed")
                frozen = private_bytes(evidence / "input.tfvars.json")
                require(digest(frozen) == journal["input_digest"], "original_input_changed")
                inputs = json.loads(frozen)
                require(args.socket == inputs["shaula"]["bindings"]["docker_host"], "docker_binding_mismatch")
                daemon_id = commands.docker_json("info", "--format", "{{json .ID}}", phase="docker_daemon_identity")
                require(daemon_id == journal["docker_daemon_id"], "docker_daemon_changed")
        ...
        except (Rejected, OSError, ValueError, KeyError, TypeError, AttributeError, IndexError, RecursionError) as error:
            report["last_phase"] = args.phase
            report["last_result"] = "failed"
            report["reason"] = str(error) if isinstance(error, Rejected) else "harness_input_or_io_error"
            save_json(Path(args.report), report)
            raise Rejected(report["reason"]) from None
        save_json(Path(args.report), report)
        print(json.dumps({"phase": args.phase, "result": "passed", "full_conformance_passed": False}))
...
if __name__ == "__main__":
    try:
        ...
        reason = str(error) if isinstance(error, Rejected) else "harness_input_or_io_error"
