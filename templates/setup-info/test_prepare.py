    ...
    def test_generates_separate_v2_sources_and_keeps_resource_shape(self):
        with tempfile.TemporaryDirectory() as directory:
            for platform in ("docker", "kubernetes"):
                ...
                generator.prepare(platform, "fixture.invalid/runner:v2@sha256:" + "a1" * 32, output)
                ...
                self.assertIn("/shaula/setup_info.json" if platform == "docker" else "cp /in/setup_info.json", main)
                self.assertEqual(main.count('resource "'), 1 if platform == "docker" else 2)
                parameters = json.loads((output / "schemas/parameters.schema.json").read_text())
                self.assertEqual(parameters["properties"]["runner_image"]["enum"], ["fixture.invalid/runner:v2"])

    def test_refuses_old_image_and_existing_output(self):
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory)
            with self.assertRaises(ValueError):
                generator.prepare("docker", "fixture.invalid/runner:v2@sha256:" + "a1" * 32, output)
