...
UI HTML、embedded assets、全部 API、health 和未来 HTTP routes 默认受保护。只有 exact GET login/callback routes 可匿名进入，以完成认证；它们不能提供应用内容。UI document 无 session 时 redirect，API/static/health 无凭据时 `401`，已认证但权限不足时 `403`。所有响应使用 private/no-store，cookie mutation 同时校验 Origin 与 CSRF。认证在 static cache/SPA fallback 和业务 handler 之前执行。
