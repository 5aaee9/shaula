...
class MockPve(ThreadingHTTPServer):
    ...
    @staticmethod
    def template(vmid):
        return {"vmid": vmid, "name": "GitHub-Runner", "node": "pve-test",
                "template": 1, "type": "qemu", "status": "stopped"}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def handle_request(self, method):
        server = self.server
        assert self.headers.get("Authorization") == "PVEAPIToken=test@pve!shaula=synthetic=secret", "API token was not split on the first equals sign"
        url = urlsplit(self.path)
        path = unquote(url.path).removeprefix("/api2/json")
        body = self.rfile.read(int(self.headers.get("Content-Length", "0")))
        form = parse_qs(body.decode()) if "multipart/" not in self.headers.get("Content-Type", "") else {}
        ...
        if method == "GET" and path == "/nodes/pve-test/storage":
            return self.reply([{"storage": "local", "type": "dir", "content": "iso", "enabled": 1, "active": 1}])
        if method == "GET" and path == "/nodes/pve-test/storage/local/content":
            return self.reply([{"volid": f"local:iso/{name}", "format": "iso", "content": "iso", "size": len(data)} for name, data in server.isos.items()])
        if method == "POST" and path == "/nodes/pve-test/storage/local/upload":
            message = BytesParser(policy=default).parsebytes(f"Content-Type: {self.headers['Content-Type']}\r\nMIME-Version: 1.0\r\n\r\n".encode() + body)
        ...
        if method == "GET" and path.startswith("/nodes/pve-test/tasks/") and path.endswith("/status"):
        ...
        if method == "POST" and path == "/nodes/pve-test/qemu/9000/clone":
            ...
            server.vms[vmid] = {"config": {"name": form["name"][0], "scsi0": f"local-lvm:vm-{vmid}-disk-0,size=8G", "net0": "virtio=BC:24:11:00:00:01,bridge=vmbr0", "ide2": f"local:vm-{vmid}-cloudinit,media=cdrom"}, "running": False}
        ...
        if path.startswith("/nodes/pve-test/qemu/"):
            ...
            if method == "PUT" and path.endswith("/config"):
                assert not {"scsi0", "net0", "cores", "memory"}.intersection(form), "Inherited hardware must not be managed"
                for key, value in form.items():
                    vm["config"][key] = int(value[0]) if key in {"onboot", "protection"} else value[0]
        ...
        if method == "DELETE" and path.startswith("/nodes/pve-test/storage/local/content/local:iso/"):
