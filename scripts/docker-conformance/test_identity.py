...
class IdentityTests(unittest.TestCase):
    def test_jit_freezes_actual_runner_identity(self):
        self.assertEqual(runner_identity(jit({"agentId": 123, "agentName": "actual"})), (123, "actual"))
        for runner in ({"agentId": True, "agentName": "actual"}, {"agentName": "actual"},
    ...
    def test_real_shaped_pascal_case_jit_with_decimal_id(self):
        ...
        runner = {"AgentId": "2", "AgentName": "shaula-codex-docker-smoke-20260908-01",
                  "Ephemeral": True, "DisableUpdate": True, "WorkFolder": "_work"}
        ...
        for id_key in ("AgentId", "agentId"):
            for name_key in ("AgentName", "agentName"):
                for identifier in (1, "1", (1 << 53) - 1, str((1 << 53) - 1)):
                    with self.subTest(id_key=id_key, name_key=name_key, identifier=identifier):
                        self.assertEqual(runner_identity(jit({id_key: identifier, name_key: "actual"})),
                                         (int(identifier), "actual"))

    def test_ambiguous_member_spellings_rejected_even_when_equal(self):
        for runner in ({"AgentId": "2", "agentId": "2", "AgentName": "actual"},
    ...
    def test_noncanonical_or_invalid_ids_rejected(self):
        invalid = (None, True, False, 0, -1, 1.0, 1.5, "", "0", "01", "+1", "-1", " 1", "1 ",
                   "1.0", "1e2", "\u0661", 1 << 53, str(1 << 53), "9" * 17)
    ...
    def test_other_runner_absence_cannot_authorize_this_container(self):
        journal = {"generation_id": "g1", "container_id": "a" * 64, "runner_id": 123, "runner_name": "actual"}
        receipt = {**journal, "repository": "5aaee9/shaula", "busy": False, "absent": True,
                   "method": "github-rest-get-404", "observed_at": datetime.datetime.now(datetime.timezone.utc).isoformat()}
        admit_removal(receipt, journal)
        for key, value in (("runner_id", 456), ("runner_name", "another"), ("generation_id", "g2"),
                           ("container_id", "b" * 64), ("busy", True), ("absent", False)):
    ...
    def test_stale_or_future_removal_receipt_rejected(self):
        ...
        for offset in (-301, 60):
            observed = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=offset)
            receipt = {**journal, "repository": "5aaee9/shaula", "busy": False, "absent": True,
                       "method": "github-rest-get-404", "observed_at": observed.isoformat()}
            with self.assertRaises(Rejected):
                admit_removal(receipt, journal)

    def test_same_address_different_saved_plan_id_rejected(self):
        resource = {"address": "docker_container.runner", "mode": "managed", "type": "docker_container",
