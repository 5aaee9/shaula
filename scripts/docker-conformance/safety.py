...
def admit_docker_engine(server):
    ...
    modules = {"github.com/moby/moby/v2", "github.com/moby/moby", "github.com/docker/docker"}
    known_engine = False
    markers = [name]
    for component in components:
        ...
        require(isinstance(component_name, str) and isinstance(module, str), "real_docker_engine_required")
        markers.extend((component_name, module))
        known_engine |= component_name == "Engine" and module in modules
    require(not any("podman" in marker.casefold() for marker in markers), "real_docker_engine_required")
    ...
    require("Docker" in name or "Moby" in name or known_engine, "real_docker_engine_required")
...
def runner_identity(encoded):
    ...
    envelope = json.loads(base64.b64decode(encoded, validate=True), object_pairs_hook=unique)
    runner = json.loads(base64.b64decode(envelope[".runner"], validate=True), object_pairs_hook=unique)
    ...
    require(type(identifier) is int and 0 < identifier <= (1 << 53) - 1, "jit_runner_id")
...
def admit_removal(receipt, journal):
    ...
    require(receipt.get("busy") is False and receipt.get("absent") is True, "github_removal_not_safe")
    require(receipt.get("repository") == "5aaee9/shaula", "removal_repository_mismatch")
    require(receipt.get("observed_at") and receipt.get("method") in ("github-rest-get-404", "github-rest-inventory"),
...
def admit_destroy_identity(plan, container_id):
    resources = [resource for resource in plan.get("resource_changes", []) if resource.get("mode") == "managed"]
    require(len(resources) == 1, "destroy_plan_identity_shape")
    resource = resources[0]
    require(resource.get("type") == "docker_container" and resource.get("address") == "docker_container.runner",
            "destroy_plan_identity_address")
    before = resource.get("change", {}).get("before")
    require(isinstance(before, dict) and before.get("id") == container_id, "destroy_plan_container_mismatch")
...
def managed_resources(document):
    def visit(module):
        ...
        result = [resource for resource in resources if resource.get("mode") == "managed"]
...
def admit_plan(plan, operation, bound_addresses):
    ...
    for resource in plan.get("resource_changes", []):
        ...
        for key in ("deposed", "importing", "previous_address"):
            require(resource.get(key) is None and change.get(key) is None, "plan_ambiguous")
    ...
    if operation == "create":
        ...
        require(not managed_resources(plan.get("prior_state", {})), "create_prior_state")
    ...
    for check in plan.get("checks", []):
        ...
        expected = {"kind": "resource", "mode": "managed", "name": "runner",
                    "to_display": "docker_container.runner", "type": "docker_container"}
        require(operation == "delete" and addresses == ["docker_container.runner"]
                and bound_addresses == ["docker_container.runner"]
                and check.get("status") == "unknown" and check.get("address") == expected
                and set(check) <= {"status", "address", "problems"}, "plan_check")
...
def inspect_container(container, expected_name, expected_image_id, jit):
    ...
    for key in ("PidMode", "IpcMode", "NetworkMode", "UTSMode", "UsernsMode"):
        ...
        require(mode != "host" and not mode.startswith("container:"), "container_host_namespace")
    ...
    require(user not in ("", "root") and not re.fullmatch(r"0+", user), "container_root_user")
    ...
    require(any(key.startswith("shaula.") for key in config["Labels"]), "container_ownership_label")
    env_names = [entry.split("=", 1)[0] for entry in config.get("Env", [])]
    for name in env_names:
        require(not name.startswith(("ACTIONS_RUNNER_INPUT_", "TF_HTTP_", "TF_VAR_", "GITHUB_TOKEN",
                                     "GH_TOKEN", "GITHUB_APP_PRIVATE_KEY", "AWS_SECRET_ACCESS_KEY")),
...
def lock_providers(lock_bytes, selected):
    ...
    blocks = re.findall(r'provider\s+"([^"]+)"\s*\{([^}]+)\}', text, re.S)
    result = []
    for source, block in blocks:
        version = re.search(r'\bversion\s*=\s*"([^"]+)"', block)
        hashes = re.search(r'\bhashes\s*=\s*\[([^]]*)\]', block, re.S)
        require(version is not None and hashes is not None, "provider_lock_shape")
        values = re.findall(r'"([^"]+)"', hashes.group(1))
        require(values and selected.get(source) == version.group(1), "provider_lock_selection")
        for value in values:
            require(re.fullmatch(r'(?:zh:[a-f0-9]{64}|h1:[A-Za-z0-9+/]{43}=)', value),
                    "provider_lock_checksum")
        result.append({"source": source.removeprefix("registry.terraform.io/"),
                       "version": version.group(1),
                       "checksums_digest": digest("".join(value + "\n" for value in sorted(values)).encode())})
