...
- saved-plan policy、provenance binding、`ApplyStarting`、`DestroyApplyStarting`、受保护输入保留和错误分类全部沿用 provider-neutral 契约；template 错误只暴露 `TemplatePlanFailed`/`TemplateExecutionFailed` 与 bounded phase。
