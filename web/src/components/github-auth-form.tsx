...
export function GitHubAuthForm({
  ...
  onAccepted,
  backendSelector,
}: AuthFormProps & { backendSelector?: React.ReactNode }) {
  ...
  async function submit(event: FormEvent) {
    ...
    setError(null);
    try {
      if (!impact.ready) throw new Error("Load the current Fleet impact before publishing.");
  ...
  return (
    ...
    >
      ...
      <div className="sticky bottom-0 z-10 flex flex-wrap items-center justify-end gap-2 border-t bg-background py-4">
        ...
        <Button type="submit" disabled={busy || !impact.ready}>
