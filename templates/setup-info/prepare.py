...
def prepare(platform, image, output):
    ...
    if digest in re.findall(r"@sha256:([0-9a-f]{64})", profile):
        raise ValueError(
            "existing v1 image pins cannot claim the new bootstrap contract"
        )
    if output.exists():
        raise ValueError(
            "output must be a new directory; existing revisions are never overwritten"
        )
    alias = image.split("@", 1)[0]
    main = (source / "main.tf").read_text()
    main, count = re.subn(
        r"(?m)^(\s*)jit_config(\s*)= string$",
    ...
    main, count = re.subn(
        r'(runner_image\s*= optional\(string, ")[^"]+("\))',
        lambda match: match[1] + alias + match[2],
        main,
    )
    if count != 1:
...
"""
        ...
        main = main.replace(
            anchor,
            "cp /in/setup_info.json /stage/setup_info.json && chmod 400 /stage/setup_info.json && "
            + anchor,
        )
        if (
            main.count('"setup_info.json" = jsonencode') != 1
            or main.count("cp /in/setup_info.json") != 1
        ):
            raise ValueError("Kubernetes bootstrap staging anchor changed")
    policy = (
        "# Setup Info bootstrap runtime policy\n\n"
    ...
    profile = re.sub(
        r"(?m)^runtime_policy_digest:.*$",
        "runtime_policy_digest: sha256:" + hashlib.sha256(policy.encode()).hexdigest(),
        profile,
    )
    ...
    (output / "schemas/parameters.schema.json").write_text(
        json.dumps(parameters, indent=2) + "\n", encoding="utf-8"
    )
    shutil.copyfile(
        source / "schemas/bindings.schema.json", output / "schemas/bindings.schema.json"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image",
        required=True,
        help="Actual newly built image repository@sha256 digest",
    )
