...
pub(super) enum ResourceDestroy {
    Destroyed { at: i64 },
    Failed { at: i64, known_failure: bool },
}
...
impl<R: RunnerRegistrations> RunnerOperation<'_, R> {
    ...
    pub(crate) async fn retire(
        ...
        if let Some(d) = &mut diagnostic.0 {
            d.observation.question.cleanup_mode = Some(CleanupMode::Ordinary);
        }
            ...
            match self.admit_registration(generation, create_started).await? {
                Registration::Removable => {
                    ...
                    match self.registrations.remove(generation, gate).await? {
                        Registration::Gone => {}
                        Registration::Busy => {
                            diagnostic.reason(Code::CleanupJobBusy, StageId::SafeRemoval, true);
                            return Ok(DestroyOutcome::Pending);
                        }
                        Registration::Unavailable => {
                            diagnostic.reason(
                                Code::CleanupSafeDrainUnavailable,
                                StageId::SafeRemoval,
                                true,
                            );
                            return Ok(DestroyOutcome::Pending);
        ...
        diagnostic.pass(StageId::SafeRemoval);
        self.walk(id, state, G::Destroying, now).await?;
        if !create_started {
            self.lifecycle
                .generation_advance(id, G::Destroyed, now)
                .await?;
            diagnostic.reason(Code::CleanupNoCreateEffect, StageId::ResourceCleanup, false);
            ...
            if let Some(d) = &mut diagnostic.0 {
                for r in &mut d.observation.question.reasons {
                    r.parameters.completion_source = Some(CompletionSource::NeverStarted);
                }
            }
        ...
        let proof = match self.destroy_proof(generation).await? {
            Ok(proof) => proof,
            Err(reason) => {
                diagnostic.reason(Code::CleanupQuarantined, StageId::Completion, true);
                return self.quarantine(id, now, reason).await;
            }
        };
        match self.destroy_resources(generation, proof, now).await? {
            ResourceDestroy::Destroyed { at } => {
                ...
                if let Some(d) = &mut diagnostic.0 {
                    for r in &mut d.observation.question.reasons {
                        r.parameters.completion_source = Some(CompletionSource::ProviderCleanup);
                    }
                }
